...
import { LayoutDashboard, Users, Film, ChevronDown, LogOut, Menu, X } from 'lucide-react';

const navItems = [
  { to: '/app', icon: LayoutDashboard, label: 'Dashboard', end: true },
  { to: '/app/roster', icon: Users, label: 'Roster' },
  { to: '/app/film-room', icon: Film, label: 'Film Room' },
];
  ...
  return (
    <div className="flex items-center gap-3">
      <svg className="w-10 h-10 shrink-0" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="dmG" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#7dd3fc" />
            <stop offset="55%" stopColor="#38bdf8" />
            <stop offset="100%" stopColor="#2563eb" />
          </linearGradient>
          <linearGradient id="dmIG" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#67e8f9" stopOpacity="0.95" />
            <stop offset="100%" stopColor="#1d4ed8" stopOpacity="0.95" />
          </linearGradient>
          <filter id="dmGl" x="-50%" y="-50%" width="200%" height="200%">
            <feGaussianBlur stdDeviation="4" result="blur" />
            <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
          </filter>
        </defs>
        <path d="M60 10 C82 10, 100 20, 110 36 L90 56 L60 88 L30 56 L10 36 C20 20, 38 10, 60 10 Z" fill="none" stroke="url(#dmG)" strokeWidth="5" filter="url(#dmGl)" />
        <polygon points="60,34 82,56 60,78 38,56" fill="none" stroke="url(#dmG)" strokeWidth="4" />
        <circle cx="60" cy="86" r="5" fill="#7dd3fc" />
        <circle cx="60" cy="56" r="11" fill="#e0f2fe" />
        <circle cx="60" cy="56" r="6" fill="#94a3b8" />
        <path d="M28 34 C42 22, 78 20, 94 34" fill="none" stroke="url(#dmIG)" strokeWidth="7" strokeLinecap="round" opacity="0.9" />
      ...
      <div className="flex flex-col leading-tight">
        ...
        <span className="text-[10px] tracking-widest uppercase" style={{ color: '#7dd3fc' }}>Sports Analytics</span>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="px-5 py-4 border-t" style={{ borderColor: '#1e3a5f' }}>
        <p className="text-xs truncate" style={{ color: '#64748b' }}>{user?.teamName}</p>
      </div>
    </>
  );
}
  ...
  return (
    <div className="flex h-screen" style={{ background: 'linear-gradient(180deg, #06122b 0%, #081a3d 100%)' }}>
      ...
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r" style={{ borderColor: '#1e3a5f', backgroundColor: 'rgba(6, 18, 43, 0.95)' }}>
        <SidebarContent user={user} onNavClick={() => {}} />
      </aside>
      ...
      >
        ...
        <SidebarContent user={user} onNavClick={() => setMobileMenuOpen(false)} />
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 shrink-0 flex items-center justify-between px-4 sm:px-6 border-b" style={{ borderColor: '#1e3a5f', backgroundColor: 'rgba(6, 18, 43, 0.6)' }}>
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div
                className="absolute right-0 top-12 w-56 rounded-xl border py-2 z-50 shadow-2xl"
                style={{ backgroundColor: 'rgba(15, 23, 42, 0.98)', borderColor: '#1e3a5f' }}
              >
