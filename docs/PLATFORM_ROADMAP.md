...
| 2026-08-12 | Phase 4.1: `zeroMeansUnmeasured` catalog flag — 0 in velocities/times/rate-percentages is treated as "not measured" at import and manual entry (warned, never stored); signed metrics and box zeros untouched | Demo sheets pre-fill 0 for non-participants; literal zeros dragged team averages down (0 mph "velocity") |
| 2026-08-12 | Stored impossible zeros cleaned by marking `excluded = 1` via admin endpoint (dry-run report first), never deleted | Reversible + auditable; the aggregate engine already ignores excluded entries |
| 2026-08-12 | "Games tracked" = scheduled/linked team games in scope; player performance records counted separately; every sample labeled with its own unit (PA / AB / IP / BF / games) | Feedback: don't conflate schedule coverage with metric sample sizes |
